...
object DbCommon {
  ...
  val createTableBooksSql: String =
    """CREATE TABLE books (
      ...
      |  FOREIGN KEY (authorId) REFERENCES authors(id));""".stripMargin
  ...
  val populateDataSql: String =
    s"""
       ...
       |
       |INSERT INTO books (id, authorId, title, year, genre) VALUES
  ...
  val fetchBooksSql: String =
    """SELECT b.id, a.id, b.title, b.year, b.genre FROM books b
      |INNER JOIN authors a ON b.authorId = a.id """.stripMargin
